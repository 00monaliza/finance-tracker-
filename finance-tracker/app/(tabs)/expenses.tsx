...
export default function ExpensesScreen() {
  ...
  const { expenses, totalExpenses, expensesByCategory, addTransaction, updateTransaction, deleteTransaction } =
    useTransactions();
  ...
  const openEdit = (tx: Transaction) => {
    setEditingTx(tx);
    setFormCategory(tx.category as ExpenseCategory);
  ...
  const handleSave = () => {
    ...
    if (editingTx) {
      ...
      addTransaction({
        ...
        amount,
        currency: '₽',
        description: formDesc.trim() || EXPENSE_CATEGORIES[formCategory].label,
  ...
  const handleDelete = (tx: Transaction) => {
    Alert.alert('Удалить расход', `Удалить «${tx.description}»?`, [
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <View style={[styles.header, { borderBottomColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.06)' }]}>
        <View>
          <ThemedText type="title">Расходы</ThemedText>
          <ThemedText style={[styles.totalLabel, { color: colors.icon }]}>
            Всего: {fmt(totalExpenses)} ₽
          </ThemedText>
        </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {/* Transaction list */}
        <ThemedText type="defaultSemiBold" style={styles.listTitle}>Последние расходы</ThemedText>
        {expenses.length === 0 ? (
          <ThemedText style={[styles.emptyText, { color: colors.icon }]}>Нет расходов</ThemedText>
        ) : (
          expenses
            .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
            .map((tx) => {
              const cat = EXPENSE_CATEGORIES[tx.category as ExpenseCategory];
              return (
                <Pressable
                  key={tx.id}
                  style={({ pressed }) => [
            ...
            <ScrollView contentContainerStyle={styles.modalContent} keyboardShouldPersistTaps="handled">
              ...
              {editingTx && (
                ...
                >
                  ...
                  <ThemedText style={styles.deleteTxt}>Удалить расход</ThemedText>
                </Pressable>
              )}
            </ScrollView>
          </KeyboardAvoidingView>
